...
st.title("PROJECT 3")#title of project3
...
constant_fk2d=st.number_input('constant_fk2d',key='1')
multiplying_factor_3d=st.number_input('multiplying_factor_3d',key='2')
Shear_velocity=2.6**(st.number_input('Shear_velocity',key='3',min_value=0))#assuming this is in this formate
def fk():
    ...
    # Shear_velocity=2.6**(st.number_input('Shear_velocity'))
...
st.subheader("Different Filtering Method")#displaying the subheading
...
if tch == 1:
    ...
    SNR=st.number_input('Enter thresold value S:',key='6',min_value=1)
elif tch == 3:
...
st.subheader('Replacement Method')#subheading
st.text('1. previous point\n2. 2*last-2nd_last\n3. overall_mean\n4. 12_point_strategy\n5. mean of previous 2 point\n6. all seqential\n7. all parallel')#asking user to chose from this only
sch=st.number_input("Chose Replacement Method From Above:",key='9',min_value=0)

if sch> 7:
    print('Please enter correct choice...')
    st.warning('Please enter correct choice...')#warning in case worng input given
else:
    if st.button('COMPUTE'):#for final computation
        fileList = open('input_file_list.txt', 'r')
        files = fileList.readlines()
        for file in files:
