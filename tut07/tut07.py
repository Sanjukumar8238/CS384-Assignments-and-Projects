...
def octant_analysis(mod=5000):
	...
	def process_file(s,mod):										# Function to process individual file
		...
		def octant_range_names(mod=5000):
			...
			ws=wb.active
			ws['M4']='Mod '+str(mod)                                           # Putting the string 'User Input' at its specified place
			...
			for i in range(13,22):                                          # Writing header list in worksheet
				ws.cell(row=3,column=i+1).value=count[i-13]
				ws.cell(row=3,column=i+1).border=thin_border
				if(i>13):
					ws.cell(row=3,column=i+9).value='Rank Octant '+str(count[i-13])
					ws.cell(row=3,column=i+9).border=thin_border
			ws.cell(row=3,column=31).value='Rank1 Octant ID'
			ws.cell(row=3,column=32).value='Rank1 Octant Name'
			ws.cell(row=3,column=31).border=thin_border
			ws.cell(row=3,column=32).border=thin_border
			...
			for i in range(13,22):                                          # Writing overall count in worksheet
				ws.cell(row=4,column=i+1).value=count[i-13]
				ws.cell(row=4,column=i+1).border=thin_border
			...
			for i in range(8):                                              # Writing overall count in worksheet
				ws.cell(row=4,column=23+i).value=rank_matrix[0][i]
				ws.cell(row=4,column=23+i).border=thin_border
				if(rank_matrix[0][i]==1):
					ws.cell(row=4,column=23+i).fill=PatternFill(start_color=yellow,end_color=yellow,fill_type="solid")
			ws.cell(row=4,column=31).value=rank1_list[0]
			ws.cell(row=4,column=32).value=octant_name_id_mapping[str(rank1_list[0])]
			ws.cell(row=4,column=31).border=thin_border
			ws.cell(row=4,column=32).border=thin_border
			...
			for i in octant:                                                # Counting number of values in different octants in mod range
				...
				if(k%mod==1):                                               # Processing the mod values in the range and storing them in the list 'count'
					...
					for i in range(13,22):                                  # Writing the mod count of octant in worksheet
						ws.cell(row=j+1,column=i+1).value=count[i-13]
						ws.cell(row=j+1,column=i+1).border=thin_border
					count.pop(0)                                            # Removing the header from list
					rank=find_rank_of_list(count)                           # Find the rank list 
					rank1_list.append(find_1st_rank(rank))                  # Finding the rank 1 octant and appending in rank1_list
					rank_matrix.append(rank)                                # Appending rank list in the matrix
					...
					for i in range(8):                                                  # Writing the columns of rank, rank1 and octant_name in the worksheet
						ws.cell(row=j+1,column=23+i).value=rank_matrix[j-3][i]
						ws.cell(row=j+1,column=23+i).border=thin_border
						if(rank_matrix[j-3][i]==1):
							ws.cell(row=j+1,column=23+i).fill=PatternFill(start_color=yellow,end_color=yellow,fill_type="solid")
		...
		def octant_longest_subsequence_count_with_range():
			...
			for i in range(3):                                              # Writing header of table to worksheet
				ws.cell(row=3,column=45+i).value=r[i] 
				ws.cell(row=3,column=45+i).border=thin_border    


			octants=[]
			for i in range(2,10,2):                                         # Writing octants on leftmost column of the table
				ws.cell(row=i+2,column=45).value=i//2
				octants.append(i//2)
				ws.cell(row=i+3,column=45).value=-(i//2) 
				octants.append(-i//2)
				ws.cell(row=i+2,column=45).border=thin_border
				ws.cell(row=i+3,column=45).border=thin_border                                
			...
			for i in range(2,10):                                           # Writing the number and length of longest subsequence in table
				ws.cell(row=i+2,column=46).value=longest_length[i-2]
				ws.cell(row=i+2,column=47).value=count[i-2]
				ws.cell(row=i+2,column=46).border=thin_border
				ws.cell(row=i+2,column=47).border=thin_border
			k=2                                                             # Variable to keep track of row in worksheet
			ws.cell(row=k+1,column=49).value='Octant ###'                          # Writing heading of table
			ws.cell(row=k+1,column=50).value='Longest Subsequence Length'
			ws.cell(row=k+1,column=51).value='Count'
			ws.cell(row=k+1,column=49).border=thin_border
			ws.cell(row=k+1,column=50).border=thin_border
			ws.cell(row=k+1,column=51).border=thin_border
			
			k+=2
			for i in range(8):
				ws.cell(row=k,column=49).value=octants[i]                   # Writing contents of table-1 for each octant
				ws.cell(row=k,column=50).value=longest_length[i]
				ws.cell(row=k,column=51).value=count[i]
				ws.cell(row=k+1,column=49).value='Time'                     # Writing header of ranges in worksheet
				ws.cell(row=k+1,column=50).value='From'
				ws.cell(row=k+1,column=51).value='To'
				ws.cell(row=k,column=49).border=thin_border                 # Adding border to cells
				ws.cell(row=k,column=50).border=thin_border
				ws.cell(row=k,column=51).border=thin_border
				ws.cell(row=k+1,column=49).border=thin_border
				ws.cell(row=k+1,column=50).border=thin_border
				ws.cell(row=k+1,column=51).border=thin_border
				x=ranges[i]
				k+=2
				for j in x:
					ws.cell(row=k,column=50).value=j[0]                     # Writing ranges in worksheet
					ws.cell(row=k,column=51).value=j[1]
					ws.cell(row=k,column=49).border=thin_border             # Adding border to cells
					ws.cell(row=k,column=50).border=thin_border
					ws.cell(row=k,column=51).border=thin_border
					k+=1
